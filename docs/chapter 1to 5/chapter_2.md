...
1.  **Localized Web Servers**: Using local gateways (such as a Raspberry Pi) to host database instances and log data locally (Kale et al., 2024). While this ensures offline reliability, it lacks the scalability and remote accessibility needed for multi-facility operations.
...
In contrast, active control systems use persistent WebSocket connections. This allows the backend to push new telemetry directly to the app UI the moment it is received, providing real-time visual updates and reducing mobile bandwidth usage by up to 60% (Nalendra & Waspada, 2025). The principles of user-centered layout and interaction design established by Adebayo Shamsudeen Anuoluwapo (2022) suggest that real-time feedback improves operator trust and system usability in high-stress agricultural applications.
...
To minimize code size and memory footprint, agricultural IoT nodes often omit device authentication (Lengkong et al., 2025). This allows attackers to send spoofed telemetry payloads directly to the backend database, potentially masking actual equipment failures and leading to hatch losses.
...
The following matrix synthesizes the features and limitations of current agricultural monitoring architectures, covering key studies in smart farming:
