...
15. **Sahoo & Pattnaik (2019)**: Built an IoT poultry farm system focusing on relay adjustments. *Limitation*: Lacked cloud logging or remote HMI. *Relevance*: Highlights the importance of localized backup triggers.
16. **TTI Cold Chain Study (2024)**: Discussed continuous temperature logging exchanges. *Limitation*: Evaluated food supply chain logistics rather than egg incubation. *Relevance*: Validates temperature logging compliance metrics.
17. **Nsengiyumva Wilberforce (2024)**: Proposed a smart agricultural framework using microcontrollers in low-resource environments. *Limitation*: No background execution or security model. *Relevance*: Confirms the viability of ESP32 platforms.
18. **Trust (2026)**: Fabricated an ESP32-based environmental chamber. *Limitation*: Lacked MQTTS wrapping and updates verification. *Relevance*: Direct predictor of hatching success rates.
19. **Broiler Monitoring Journal (2024)**: Tested temperature/humidity controls in large-scale chicken coops. *Limitation*: High complexity, no mobile integration. *Relevance*: Informs relative humidity target boundaries.
20. **Kumar & Singh (2018)**: Reviewed local web servers for agricultural parameters monitoring. *Limitation*: High hardware crash rate. *Relevance*: Justifies the transition to FastAPI cloud hosting.
21. **Verma & Ranga (2018)**: Analyzed security gaps in WSN nodes. *Limitation*: Purely theoretical paper. *Relevance*: Informs access control architectures.
22. **Wortmann & Flüchter (2015)**: Explored IoT lifecycle systems and remote updates. *Limitation*: Enterprise-focused, expensive. *Relevance*: Shapes firmware version notification model.
23. **Soeb et al. (2021)**: Fabricated a low-cost incubator. *Limitation*: Lacked wireless connectivity or remote UI. *Relevance*: Confirms developmental temperature baselines.
24. **Malaysia IoT Study (2020)**: Demonstrated a simple coops monitor using Blynk. *Limitation*: Network drops resulted in data gaps. *Relevance*: Informs the design of offline buffer queues.

### 2.6.2 Thematic Summary Matrix (25 Core Papers Synthesis)
The following comparative matrix synthesizes the systems architectures, alerting, buffering, security postures, and identified gaps across 25 core papers reviewed in this study:
...
| TTI Study (2024) | Time-Temperature Indicators | None | Passive indicator | Physical tag | No active telemetry, no real-time mobile alerts |
| Wilberforce (2024) | Arduino + Wi-Fi | Local screen | No | None | Lacks secure cloud database logging and mobile app alerts |
| Trust (2026) | ESP32 + DS18B20 | Local Buzzer | No | WPA2 | Omites transport layer encryption (MQTTS) and remote update notifications |
| K. Ramanan (2025) | PLC + Industrial Gateway | SCADA alarm | Yes | Password | High deployment cost, proprietary architecture, lacks consumer mobile HMI |
| Broiler Journal (2024) | WSN + Gateway | Local buzzer | No | None | Focuses on physical measurements rather than secure remote app access |
| Kumar & Singh (2018) | Arduino + Web | None | No | HTTP Auth | High risk of local server failures, no mobile push notification layer |
| Verma & Ranga (2018) | Conceptual Security | None | None | Crypto review | Review paper; does not implement a physical incubator sensing prototype |
| Wortmann & (2015) | Industrial IoT | Dashboard | Yes | Certificate | Heavy enterprise focus, unsuitable for low-cost smallholder farming systems |
| Soeb et al. (2021) | Microprocessor + Relays | LCD warning | No | None | Lacks wireless connectivity, cloud storage, or remote monitoring interfaces |
| Malaysia Study (2020) | NodeMCU + Blynk | Blynk Push | No | Blynk Token | No offline data resilience, lacks transport-level TLS encryption |
