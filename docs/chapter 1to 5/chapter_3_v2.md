...
1.  **Users Table**: Stores user authentication credentials.
    ...
    *   `role` (String): Operator or Admin access level.
2.  **Temperature Records Table**: Logs temperature data.
    *   `id` (PK, Integer): Unique identifier.
    *   `sensor_id` (String): Hardware ID of the DS18B20 probe.
    *   `temperature` (Float): Captured temperature reading.
    *   `timestamp` (DateTime): Log creation time.
3.  **Security Alerts Table**: Logs security events.
    *   `id` (PK, Integer): Unique identifier.
    *   `alert_type` (String): Warning classification.
    *   `message` (String): Alert message.
    *   `timestamp` (DateTime): Event time.
4.  **Notifications Table**: Tracks push notifications sent to the user.
    *   `id` (PK, Integer): Unique identifier.
    *   `user_id` (FK, Users.id): Recipient user.
    *   `title` (String): Message title.
    *   `body` (String): Message body.
    *   `status` (String): Delivered, Pending, or Read.
