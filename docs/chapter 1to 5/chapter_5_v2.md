...
The empirical results of the Egg Guardian system demonstrate significant improvements in accuracy, latency, and data resiliency compared to traditional agricultural monitoring systems:

### 5.2.1 Thermal Monitoring Precision
The DS18B20 digital probe achieved a Mean Absolute Error (MAE) of **$0.24^\circ\text{C}$** during calibration (Table 4.1). In contrast, common analog thermal monitoring systems using DHT11 or LM35 sensors often exhibit measurement errors exceeding $\pm 1.0^\circ\text{C}$ due to voltage fluctuations and signal attenuation over long cables (Nalendra, 2025). Because poultry egg embryos are highly sensitive to temperature variations outside the safe range of $37.0^\circ\text{C}$ to $39.0^\circ\text{C}$, the digital 1-Wire DS18B20 probe is more suitable for this application. It provides reliable noise-resilient readings, which are essential for maintaining hatch rates (FAO, 2024).

### 5.2.2 End-to-End Latency and Alert Delivery
The alert latency benchmarks (Table 4.2) showed an average propagation delay of **$1.14\text{ seconds}$** over local Wi-Fi and **$1.41\text{ seconds}$** over mobile 4G links. Traditional SMS/GSM-based alerts used in smart agriculture often suffer queue congestion and routing delays, resulting in latency spikes of 10 to 60 seconds (Gatkal et al., 2025). By using persistent WebSocket connections and the Firebase Cloud Messaging (FCM) gateway, the Egg Guardian system delivers real-time notifications quickly. This enables operators to respond rapidly to critical heating or cooling failures.

### 5.2.3 Data Resiliency and Network Recovery
The edge node's local FIFO caching queue achieved a **100% data recovery rate** for Wi-Fi drops lasting up to 100 seconds (Table 4.3). Traditional agricultural IoT dashboards (such as Blynk or ThingSpeak) typically operate without edge buffers, resulting in complete telemetry data loss during network dropouts (Pacheco da Costa et al., 2023). By caching readings on the ESP32 and flushing them upon reconnection, the Egg Guardian system ensures historical logs remain complete. This is crucial for verifying continuous thermal compliance.

### 5.2.4 Firmware Lifecycle and Security Warning loops
The backend version check completed in **$120\text{ms}$**, triggering mobile warning banners and FCM notifications immediately upon mismatch detection. Most existing agricultural microcontrollers operate without firmware version checks, leaving them vulnerable to outdated software exploitation and code injection (Dr. Brindha S et al., 2025). By verifying firmware versions during the broker handshake, the Egg Guardian system secures edge devices and alerts operators to required updates.
...
## 5.4 Practical and Commercial Implementation Issues
...
## 5.5 Research Contributions to Agricultural Computing
